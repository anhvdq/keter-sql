...
public class Join extends Relation {
    ...
    public Join(Type type, Relation left, Relation right, JoinCriteria criteria) {
        this(null, type, left, right, criteria);
    }
    ...
    public Join(NodeLocation location, Type type, Relation left, Relation right,
        ...
        if ((type == Type.CROSS) || (type == Type.IMPLICIT)) {
            checkArgument(criteria == null, "%s join cannot have join criteria", type);
        } else {
            checkArgument(criteria != null, "No join criteria specified");
        }
    ...
    public List<Node> getChildren() {
        ...
        nodes.add(left);
        nodes.add(right);
        Optional.ofNullable(criteria).map(JoinCriteria::getNodes).ifPresent(nodes::addAll);
        return nodes.build();
    }

    @Override
    public String toString() {
        return toStringHelper(this).add("type", type).add("left", left).add("right", right).add("criteria", criteria)
    ...
    public boolean equals(Object o) {
        ...
        Join join = (Join) o;
        return (type == join.type) && Objects.equals(left, join.left) && Objects.equals(right, join.right)
